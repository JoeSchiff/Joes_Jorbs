...
def build_keyword_list(keyword_array, keyword_list):
    ...
    for keyword in keyword_array.split('\x1f,'):  # Split array into keywords based on delim char
        keyword = keyword.rstrip('\x1f').lower()  # Try to remove delim char because the last kw in the array does not have a comma after
...
def get_keywords():
    ...
    field_keyword = FieldStorage().getvalue('kw_form')  # Keyword remaining in the input field
    keyword_list = build_keyword_list(field_keyword, keyword_list)

    keyword_array_str = FieldStorage().getvalue("kw_list")  # Keyword(s) in array from "Enter" button
...
def sort_keywords_into_fm_lists(keyword_list):
    ...
    for keyword in keyword_list:
        if len(keyword) > 17: two_edits_l.append(keyword)
        elif len(keyword) > 8: one_edits_l.append(keyword)
        else: zero_edits_l.append(keyword)
...
def create_regex_pattern(keyword_list):
    if FieldStorage().getvalue('fuzz_cb'):
        reg_pat = ''
        zero_edits_l, one_edits_l, two_edits_l = sort_keywords_into_fm_lists(keyword_list)
...
def calc_percent_similarity(reg_res):
    num_edits = sum(reg_res.fuzzy_counts)  # Total substitutions, insertions, and deletions
    fuzzy_percent = round(len(reg_res.group()) / (len(reg_res.group()) + num_edits) * 100, 1)  # "group" is the matched str
...
def build_results_list(checked_list):
    ...
    for org_name, file_path, url in checked_list:
        ...
        reg_res = regex.search(reg_pat, file_text)  # Search text file for compiled pattern
        ...
        if reg_res:
            ...
            res_list.append([org_name, url, jbw_conf, str(fuzzy_percent), reg_res.group()])

    return sorted(res_list, key = lambda x: int(x[2]), reverse=True)  # Sort results by jbw conf



def display_results(res_list):
    print('<div id="Results" class="tabcontent" style="background-color: #132020">')
    print(f'<br>Keywords used: {str(keyword_list)[1:-1]} <h3> {len(res_list)} Matches found at:</h3>')
    ...
    for org_name, url, jbw_conf, fuzzy_percent, match_s in res_list:
        if org_name not in org_tracker_l and url not in url_tracker_l:  # Display org name only once and only when a URL will be displayed
            print(f'<br><br><strong> {org_name} </strong>')
            org_tracker_l.append(org_name)
        if url not in url_tracker_l:
            print(f'<br>&emsp;&emsp;&emsp;&emsp;<a style="text-decoration:none;" href="{url}" title="Keyword match: {match_s}&#10;Percent similar: {fuzzy_percent}%">{url}</a>')
...
def display_checked_pages(checked_list):
    print(f'<div id="Pages_checked" class="tabcontent"><br><h3>These {len(checked_list)} pages were successfully searched</h3>')
    org_tracker_l = []
    ...
    for org_name, file_path, url in sorted(checked_list):
        if not org_name in org_tracker_l:
            print(f'<br><br><strong> {org_name} </strong>')
            org_tracker_l.append(org_name)
        print(f'<br>&emsp;&emsp;&emsp;&emsp;<a style="text-decoration:none;" href="{url}">{url}</a>')
    print('<br><br><br></div>')
...
def sort_error_urls(errorlog_d):
    ...
    for url, err_v in errorlog_d.items():
        ...
        if not err_v[-1][-1] == 'jj_final_error': continue

        db_type = err_v[0][1]
        org_name = err_v[0][0]

        # Sort errors to portal or regular based on crawl level
        if err_v[0][2] == 0:  # change to "< 1" to include domain fallback urls. this would cause dups?
...
def display_portal_errors(port_err_display_l):
    print(f'<div id="Domain_errors" class="tabcontent"><br><h3>The program was unable to search any webpages on these {len(port_err_display_l)} domains</h3>')
...
def display_reg_errors(reg_err_display_l):
    print(f'<div id="Errors" class="tabcontent"><br><h3>The program was unable to search these {len(reg_err_display_l)} webpages</h3>')
...
def calc_error_rate(checked_list, reg_err_display_l, port_err_display_l):
    if len(checked_list) < 1:
        ...
        error_rate = (len(reg_err_display_l) + len(port_err_display_l)) / len(checked_list)
...
def display_stats(keyword_list, dater, display_db_l, home_zip, home_coords, max_dist, checked_list, port_err_display_l, port_err_l, reg_err_display_l, reg_err_l, error_rate, error_rate_desc, reg_pat):
    ...
    '<br>Scraping date:', dater.split('/')[-1], 
    '<br>Databases:', display_db_l, 
    '<br>ZIP code:', home_zip, 
    '<br>Coordinates:', home_coords, 
    '<br>Max distance:', max_dist, 
    'miles<br><br>Pages searched:', len(checked_list), 
    ...
    ')<br>Total error rate:', f'{round(error_rate * 100, 1)}%', error_rate_desc, 
    '<br>Script duration:', duration, 'seconds', 
    '<br>Regex pattern:', reg_pat.pattern, '<br><br><br></strong></div>')
...
if __name__ == '__main__':
    ...
    error_rate, error_rate_desc = calc_error_rate(checked_list, reg_err_display_l, port_err_display_l)

    display_stats(keyword_list, dater, display_db_l, home_zip, home_coords, max_dist, checked_list, port_err_display_l, port_err_l, reg_err_display_l, reg_err_l, error_rate, error_rate_desc, reg_pat)
